                    ...
                    for next_group_id in range(group_id + 1, data.group + 1):
                        for player_id in range(1, data.players):
                            for next_player_id in range(player_id + 1, data.players + 1):
                                ...
                                variable_other_week = data.generate_minimal_variable(player_id, next_group_id, next_week_id)
                                variable_next_other_week = data.generate_minimal_variable(next_player_id, next_group_id, next_week_id)
